...
<h3>Детали заказов</h3>
					...
					while($detail_row = mysqli_fetch_assoc($detail_result)){
						if(!empty($detail_row['product_id'])){
							...
							echo "</div>
								<div class=\"totalDetails\">
									...
									<p>Статус заказа: "; if($row['order_state'] == 1){echo "Создан";} 
														...
														elseif($row['order_state'] == 4){echo "Отправлен";}											
														elseif($row['order_state'] == 5){echo "Выдан получателю";}											
									echo "</p>
							...
							if($row['order_state'] == 1){
								echo "<a href=\"myOrder.php\">Перейти к оплате</a>";
							}
						}
					}
	...
	<br>
	<br>
	<a href="personalAccount.php">Вернуться</a>
